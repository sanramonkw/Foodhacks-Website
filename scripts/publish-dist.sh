#!/usr/bin/env bash
# Publishes the already-built combined dist/ (master, built with
# DEPLOY_TARGET=pages, + the premium/editorial variants) to the repo's
# gh-pages branch. Windows-safe: unlike the `gh-pages` npm package (which
# passes the whole file list to `git rm` and dies with ENAMETOOLONG on
# Windows), this runs `git add -A` from *inside* dist/, so there is no giant
# argument list.
#
