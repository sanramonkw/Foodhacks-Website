...
npm run build     # -> dist/  (must pass; last verified 2026-07-02, 9 pages)
npm run preview
```
...
- **Nav** was Regular with a green "active + button Order Now" — live is FontHead2 15px black,
  ...
  Now" is a plain menu item, not a button. Live nav also has an `العربية` item — deliberately
  omitted until `/ar/` is migrated.
...
(live pages are title-banner-only — documented in Content notes); no `العربية` nav item yet;
AOS scroll-fade animations not replicated (content is simply always visible).
...
Nav also carries the external **Order Now** link: https://srkw.co/en/product-category/food-hacks/
...
### Pending migration (full inventory from WP sitemaps)
- **Arabic mirror** (Polylang `/ar/...`): home `الرئيسية`, `about-ar`, `products-ar`, `faqs-ar`,
  `contact-ar`, `return-refund-policy-ar`, 3 AR product pages, 5 AR FAQ items. Largest gap.
- WP `footer-sitemap.xml` items (footer-logo/subscribe/contact/social widgets, EN+AR) — content
  already folded into `Footer.astro`; nothing else to migrate there.
...
- `public/llms.txt` — brand/product/facts summary for generative engines.
- hreflang alternates NOT yet emitted (blocked on Arabic migration; add
  `<link rel="alternate" hreflang>` pairs in BaseLayout when /ar/ exists).
...
2. **Arabic site** (`/ar/...`) — full mirror exists on WP; either Astro i18n routing
   (`i18n` config, `src/pages/ar/...`) or a content-collection-per-locale approach. Then add
   hreflang + `og:locale:alternate`, and dir="rtl" layout variant.
3. **Redirects** — WP FAQ/product URLs that changed shape (e.g. `/faq/<slug>/` single pages)
   should 301 to `/faqs/`; keep `/product/<slug>/` URLs (already identical).
4. Product body copy expansion (owner input) + real product photos beyond the 3 renders.
5. Newsletter — footer promises one; hook up a provider (Mailchimp/Buttondown) or remove the blurb.
6. Meta Pixel/analytics: the WP site had none on foodhacks.co pages themselves; ask owner if
   analytics (e.g. Plausible) is wanted.
7. Favicon: currently the original PNG; consider adding ICO/apple-touch sizes.
...
src/content.config.ts   # products + faqs collections (zod)
src/content/            # 3 products, 5 faqs (markdown)
src/layouts/BaseLayout.astro   # SEO head + JSON-LD graph builder
src/components/         # Header, Footer, PageHero, ProductCard
