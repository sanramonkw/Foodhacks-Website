...
function buildMarqueeTag(text, isAr) {
  const span = document.createElement('span');
  span.className = isAr ? 'mtag is-ar' : 'mtag';
  if (isAr) span.setAttribute('dir', 'rtl');
...
function updateMarquee(lang) {
  ...
  track.classList.toggle('is-rtl', isAr);
  track.replaceChildren();
  [...items, ...items].forEach(text => track.appendChild(buildMarqueeTag(text, isAr)));
...
function initMarquee() {
  ...
  if (!half || half % 1 !== 0) return;

  const shift = items[half].offsetLeft - items[0].offsetLeft;
  if (shift <= 0) return;

  track.style.setProperty('--marquee-shift', `${shift}px`);

  const isMobile = window.innerWidth <= 767;
  const pxPerSec = isMobile ? 90 : 100;
  track.style.animationDuration = `${Math.max(isMobile ? 14 : 18, shift / pxPerSec)}s`;
...
(function(){ const s = localStorage.getItem('fh-lang'); if(s && s !== 'en') applyLang(s); else updateMarquee(currentLang); })();

let marqueeResizeTimer;
window.addEventListener('resize', () => {
  clearTimeout(marqueeResizeTimer);
  marqueeResizeTimer = setTimeout(initMarquee, 150);
}, { passive: true });

if (document.fonts?.ready) document.fonts.ready.then(initMarquee);
